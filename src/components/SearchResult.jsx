    ...
    return (
        <div className="flex flex-col min-h-[100vh]">
            ...
            <main className="grow p-[12px] pb-0 md:pr-5 md:pl-20 ">
                ...
                {!imageSearch? (
                    <>
                    {items.map((item, index) => (<SearchedItemTemplate key={index} data ={item}/>
                    ))}
                    </>
                ) :(
                    <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-6 gap-5">{items.map((item,index)=>(<SearchedImageItemTemplate key = {index} data={item}/>))}</div>
